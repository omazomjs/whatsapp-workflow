...
- **16/09 (noche) — Repositorio empresarial, borrado de usuarios y manual**:
  - Repo Forgejo empresarial "Nidavellir" creado: `http://192.168.5.87:3000/omazo/whatsapp-workflow` (privado), remote `empresa`, push del estado actual (`6a2ce5a`). `origin` sigue apuntando a GitHub.
  - **Borrado definitivo de usuarios del panel**: nuevo botón **Borrar** (junto a "Desactivar") que elimina el registro sin vuelta atrás. solo admin; prohibido borrarse a sí mismo (guard en API). Backend: `borrarUsuarioDefinitivo()` + `buscarUsuarioPorId()` en `src/database.js`; `DELETE /api/usuarios/:id` acepta `{ definitivo: true }`. El "Desactivar" (borrado lógico) se conserva.
  - **`MANUAL_PANEL.pdf`** en la raíz del repo: manual de usuario (acceso HTTPS 3443, alarmas DIARIA/INMEDIATA anti-baneo, tipo pendientes, resumen diario configurable, usuarios, mantenimiento, URLs). Generado desde HTML con Chrome headless.
  - **Botón "i" azul del manual** en la cabecera del panel: abre `/manual.pdf`. Se sirve como **estático** en `web/public/manual.pdf` (copiado de `MANUAL_PANEL.pdf`) → **funciona sin reiniciar** el panel (express.static lee de disco). Ruta alternativa en `web/server.js` (`/manual.pdf` con `requiereSesion` + sendFile de la raíz) como reserva tras reinicio.
  - **Manual de administración restringido** (`MANUAL_ADMIN.pdf`, ruta `/manual-admin.pdf`): contenido avanzado (arquitectura, SQL, despliegue, troubleshooting, certs, frenos anti-baneo). Botón `<A>` rojo oscuro en cabecera (`.boton-info.admin`), solo visible para `dfueyo` y `omazo` (check en `app.js` mostrarApp + guard en `web/server.js` con `ADMIN_MANUAL_USERS`). **Requiere reinicio de `WhatsAppWeb`** para activar la ruta (el estático no sirve este archivo porque está en la raíz, no en `web/public`).
  - Desplegado en servidor: `src/database.js`, `web/server.js`, `web/public/*.{app,index,estilos}.js/html/css`, `MANUAL_PANEL.pdf`. **Pendiente reiniciar `WhatsAppWeb`** para que cargue el cambio.
...
- **15/09 — Casa: Fase 1b · Alarmas INMEDIATAS + tipo `pendientes`/estancadas + branding corporativo**:
  ...
  - **Nuevo tipo `pendientes`** (facturas estancadas): `evaluarPendientes()` con criterios `importeMin`, `diasMin`, `IDEstado` — la funcionalidad solicitada de detectar facturas estancadas. Se ejecuta a hora fija (sin modo inmediato).
